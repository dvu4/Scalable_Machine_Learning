...
def makePlural(word):
    ...
    """
    return map(lambda w1: ''.join((w1,'s')),word)
    #return word + 's'
...
# TODO: Replace <FILL IN> with appropriate code
pluralLambdaRDD = wordsRDD.map(lambda w1: ''.join((w1,'s')))
...
# TODO: Replace <FILL IN> with appropriate code
#w[0] is key
#w[1] is list of values
wordCountsGrouped = wordsGrouped.map(lambda w: (w[0], sum(w[1])))
...
def wordCount(wordListRDD):
    ...
    """
    wordCountsCollected = wordListRDD.map(lambda w: (w,1)).reduceByKey(lambda x,y: x+y)
    #print wordCountsCollected
...
# TODO: Replace <FILL IN> with appropriate code
top15WordsAndCounts = wordCount(shakeWordsRDD).takeOrdered(15,key = lambda (k,v): -v)  # descending order --> key=lambda x: -x
