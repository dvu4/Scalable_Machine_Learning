...
def parsePoint(line):
    ...
    """
    #temp = line[0].split(',')
    #label, features = temp[0], temp[1:]
    #return LabeledPoint(label, features)
    return LabeledPoint(line.split(',')[0], line.split(',')[1:])

### each element in list parsedSamplePoints is LabeledPoint object
# parsedSamplePoints = [parsePoint(line) for line in samplePoints]
# parsedSamplePoints = map(lambda line: parsePoint(line),samplePoints)
parsedSamplePoints = map(parsePoint,samplePoints)

for i in parsedSamplePoints:
    print i.label
...
# TODO: Replace <FILL IN> with appropriate code
#parsedData = parsedDataInit.map(lambda x: x.label)
...
def squaredError(label, prediction):
    ...
    #return np.power((prediction - label),2)
...
# In this part, we use the baseline model, so the prediction values for train, 
# validation and test data are average label --> averageTrainYear
...
alpha = 1.0
miniBatchFrac = 1.0
for reg in np.array([1e-10, 1e-5, 1]):
...
def twoWayInteractions(lp):
    ...
    #two_way_inter = list(itertools.product(range(len(lp.features)),range(len(lp.features))))
    two_way_inter = list(itertools.product(range(len(lp.features)),range(len(lp.features))))
    #return (lp.label,np.hstack(lp.features,[map(lambda (i,j) : lp.features[i]*lp.features[j],two_way_inter)]))
    return LabeledPoint(lp.label, np.hstack((lp.features,[lp.features[i] * lp.features[j] for (i,j) in two_way_inter])))
...
rmseTestInteract = rmseValLR1 = calcRMSE(labelsAndPredsTest)
